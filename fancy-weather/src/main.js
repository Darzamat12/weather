...
function updateTime() {
    ...
    futureDays = document.querySelector('.future-weather').childNodes;
    let j = 1;
    for (let i = 1; i < futureDays.length; i += 2) {

        futureDays[i].childNodes[1].textContent = (date.getDay() + j) < 7 ? days[date.getDay() + j] : days[date.getDay() + j - 7];
        j++;
    }
...
async function getLinkToImage() {
    ...
    img = await resJson;
    document.querySelector('body').style.backgroundImage = `url(${img.urls.regular}`;
}
...
async function getTemp() {
    ...
    mapboxgl.accessToken = 'pk.eyJ1IjoiZGFyemFtYXQiLCJhIjoiY2s0OHlheXlvMGNsYjNmbXB0dWZydHdmeiJ9.FDEByYGutcoK_OeX4oy0Aw';
    var map = new mapboxgl.Map({
        container: 'map', // container id
        style: 'mapbox://styles/mapbox/streets-v11', // stylesheet location
        center: [dataTemp.city.coord.lon, dataTemp.city.coord.lat], // starting position [lng, lat]
...
recognizer.onresult = function(event) {
    ...
    if (result.isFinal) {
        ...
        document.querySelector('.search-city').value = result[0].transcript;
    } else {
